...
var require_zh_CN_schema_plugin = __commonJS({
  "src/locales/zh-CN.schema.plugin.yml"(exports, module) {
    module.exports = { $inner: [{ $desc: "全局配置", $inner: { chatConcurrentMaxSize: "当前适配器支持的模型最大并发聊天数。", chatTimeLimit: "每小时的 API 调用次数限制。", maxRetries: "模型请求失败后的最大重试次数。", timeout: "模型请求的超时时间（毫秒）。", configMode: { $desc: "请求配置模式。", $inner: ["顺序配置模式（当前配置无效时，自动切换至下一个可用配置）。", "负载均衡模式（轮询使用所有可用配置）。"] }, proxyMode: { $desc: "当前插件的代理设置模式。", $inner: ["遵循 ChatLuna 主插件的全局代理设置", "禁用代理", "使用自定义代理设置"] } } }, { proxyAddress: "当前插件的自定义代理地址。仅在使用自定义代理设置时生效。" }] };
  }
});
...
var MessageTransformer = class {
  ...
  async transform(session, elements, model, message = {
    ...
    if (session.quote && !options.quote && options.includeQuoteReply && sourceElementString !== quoteElementString) {
      ...
      const extractImages = /* @__PURE__ */ __name((content) => Array.isArray(content) ? content.filter((item) => isMessageContentImageUrl(item)) : [], "extractImages");
...
async function readArchivePayload(archivePath) {
  ...
  if (stat.isDirectory()) {
    ...
    if (manifest.checksum != null && manifest.checksum.length > 0) {
      const checksum = createHash("sha256").update(messageBuffer).digest("hex");
...
var ConversationService = class {
  ...
  async exportConversation(session, options = {}) {
    ...
    const outputPath = options.outputPath ?? path2.join(
      await this.ensureDataDir("export"),
      `${conversation.id}-${Date.now()}.md`
    );
    await fs2.writeFile(outputPath, markdown, "utf8");
    return {
      conversation,
      path: outputPath,
  ...
  async archiveConversationById(conversationId, inactiveBefore) {
    ...
    return this.runtime.withConversationSync(conversation, async () => {
      ...
      const checksum = createHash2("sha256").update(messageBuffer).digest("hex");
      const now = /* @__PURE__ */ new Date();
      const manifest = {
      ...
      await fs2.writeFile(
        path2.join(archiveDir, "conversation.json"),
        JSON.stringify(payload.conversation, null, 2),
        "utf8"
      );
      await fs2.writeFile(
        path2.join(archiveDir, "messages.jsonl.gz"),
        messageBuffer
      );
      await fs2.writeFile(
        path2.join(archiveDir, "manifest.json"),
  ...
  async restoreConversation(session, options = {}) {
    ...
    if (archive == null) {
      throw new ChatLunaError3(
        ChatLunaErrorCode3.CONVERSATION_ARCHIVE_NOT_FOUND
      );
    }
    if (archive.conversationId !== conversation.id) {
      throw new ChatLunaError3(
        ChatLunaErrorCode3.CONVERSATION_ARCHIVE_MISMATCH
      );
    }
    if (managed?.lockConversation ?? resolved.constraint.lockConversation) {
      throw new ConstraintLockedError("restore");
    ...
    return this.runtime.withConversationSync(conversation, async () => {
      ...
      try {
        ...
        if (updated == null) {
          throw new ChatLunaError3(
            ChatLunaErrorCode3.CONVERSATION_RESTORE_FAILED
          );
        }
        if (options.allPresetLanes && getLookupKeys(
          session,
  ...
  async ensureDataDir(name) {
    const target = path2.resolve(this.ctx.baseDir, "data/chatluna", name);
...
var ConversationRuntime = class {
  ...
  interfaces = new LRUCache({
    max: 20,
    dispose: /* @__PURE__ */ __name((value) => {
      value.chatInterface.dispose?.();
    }, "dispose")
  });
  modelQueue = new RequestIdQueue();
  conversationQueue = new RequestIdQueue();
    ...
    if (config.timeout > 0) {
      releaseIdleTimer = this.service.ctx.setInterval(
        () => {
          ...
          abortController.abort(
            new ChatLunaError4(
              ChatLunaErrorCode4.API_REQUEST_TIMEOUT,
              void 0,
              true
            )
          );
...
${reasoning}`
  ...
  async withConversationAndPlatformLock(conversation, callback) {
    const requestId = randomUUID2();
    const modelRequestId = randomUUID2();
    const platform = requirePlatform(conversation);
    ...
    if (client.value == null) {
      throw new ChatLunaError4(
        ChatLunaErrorCode4.CONVERSATION_PLATFORM_NOT_AVAILABLE,
  ...
  stopRequest(requestId) {
    ...
    active.abortController.abort(
      new ChatLunaError4(ChatLunaErrorCode4.ABORTED, void 0, true)
    );
    return true;
  }
...
function requirePlatform(conversation) {
  ...
  if (platform == null) {
    throw new ChatLunaError4(
      ChatLunaErrorCode4.CONVERSATION_INVALID_MODEL,
...
import { DefaultRenderer } from "koishi-plugin-chatluna";
...
var ChatLunaPromptRenderService = class {
  ...
  _initBuiltinFunctions() {
    this.registerFunctionProvider("time_UTC", (args) => {
      ...
      if (isNaN(utcOffset)) {
        logger3.warn(`Invalid UTC offset: ${args[0]}`);
    ...
    return await Promise.all(
      messages.map(async (message) => {
        const content = await this.renderTemplate(
          getMessageContent2(message.content),
          variables,
          options
        );
...
import { ChatLunaContextManagerService as ChatLunaContextManagerService2 } from "koishi-plugin-chatluna/llm-core/prompt";
...
import { BaseMessage as BaseMessage2, HumanMessage as HumanMessage3 } from "@langchain/core/messages";
var STAGE_ORDER = {
  system_prompts: 0,
...
function toMessages(input) {
  ...
  if (typeof input === "string") {
    ...
    return [new HumanMessage3(input)];
  }
  return [];
}
...
var ChatLunaService = class extends Service {
  constructor(ctx, config) {
    ...
    this._contextManager = new ChatLunaContextManagerService2(ctx);
  ...
  async installPlugin(plugin) {
    ...
    if (this._plugins[platformName]) {
      throw new ChatLunaError5(
        ChatLunaErrorCode5.PLUGIN_ALREADY_REGISTERED,
  ...
  async awaitLoadPlatform(plugin, timeout = 3e4) {
    ...
    this[Context3.origin].effect(() => () => disposable.stop());
    return promise;
  }
  ...
  async createChatModel(platformName, model) {
    ...
    if (model == null) {
      ;
      [platformName, model] = parseRawModelName3(platformName);
    }
  ...
  async createEmbeddings(platformName, modelName) {
    ...
    if (modelName == null) {
      ;
      [platformName, modelName] = parseRawModelName3(platformName);
    }
    ...
    return computed(() => {
      ...
      if (info.value.type !== ModelType2.embeddings) {
        this.ctx.logger.warn(
  ...
  async createAgent(options) {
    const llm = await resolveAgentModel(
      options.model,
    ...
    const { preset, instructions } = resolveAgentPreset(
      options,
      (name) => computed(() => this._preset.getPreset(name).value)
    );
    const model = llm.value;
    const prompt = options.prompt ?? new ChatLunaChatPrompt({
      preset,
  ...
  _createTempDir() {
    const tempPath = path3.resolve(this.ctx.baseDir, "data/chatluna/temp");
    if (!fs3.existsSync(tempPath)) {
      fs3.mkdirSync(tempPath, { recursive: true });
    }
  }
...
var ChatLunaPlugin = class {
  constructor(ctx, config, platformName, createConfigPool = true) {
    ...
    if (createConfigPool) {
      ...
      this.platformConfigPool = new ClientConfigPool(
        ctx,
  ...
  createRunnableConfig() {
    ...
    const abort = /* @__PURE__ */ __name(() => abortController.abort(
      new ChatLunaError5(ChatLunaErrorCode5.ABORTED, void 0, true)
    ), "abort");
