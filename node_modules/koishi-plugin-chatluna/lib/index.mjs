...
import { h as h2, Universal } from "koishi";
...
function applyAgentTaskWakeup(ctx, config) {
  ctx.on("chatluna/agent-task-finished", async (payload) => {
    ...
    const msg = new HumanMessage4({ content, name: "task" });
...
function restoreSession(ctx, payload) {
  ...
  if (bot == null || bot.status !== Universal.Status.ONLINE) return void 0;
...
import {
  callChatLunaChain,
  ChatLunaLLMChain as ChatLunaLLMChain2,
...
import { getMessageContent as getMessageContent4 } from "koishi-plugin-chatluna/utils/string";
var ChatLunaChatChain = class _ChatLunaChatChain extends ChatLunaLLMChainWrapper {
  ...
  static fromLLM(llm, {
    ...
    const chain = new ChatLunaLLMChain2({ llm, prompt });
  ...
  async call({
    ...
    requests["variables"] = Object.assign(variables ?? {}, {
      prompt: getMessageContent4(message.content)
    });
    ...
    const aiMessage = response.message ?? new AIMessage3(response.text);
...
var ChatLunaPluginChain = class _ChatLunaPluginChain extends ChatLunaLLMChainWrapper2 {
  ...
  async call({
    ...
    requests["variables"] = Object.assign(nextVars, {
      prompt: getMessageContent5(message.content)
    });
    requests["variables"]["built"] = {
      conversationId,
      requestId,
    ...
    if (response == null) {
      throw new ChatLunaError3(ChatLunaErrorCode3.API_REQUEST_FAILED);
    }
    return response;
  }
  get model() {
...
async function defaultFactory(ctx, service) {
  ...
  ctx.on("chatluna/model-removed", async (_service, platform) => {
    const tasks = ctx.chatluna.conversationRuntime.getCachedConversations().filter(
      ([_, entry]) => parseRawModelName(entry.conversation.model)[0] === platform
    ).map(async ([id, entry]) => {
      const result = await ctx.chatluna.conversationRuntime.clearConversationInterface(
        entry.conversation
      );
      if (result) {
        logger6?.debug(`Cleared cache for conversation ${id}`);
      }
    });
...
import { logger as logger7 } from "koishi-plugin-chatluna";
...
async function runRoomToConversationMigration(ctx, config) {
  const hasSentinel = existsSync2(getLegacySchemaSentinel(ctx.baseDir));
...
async function migrateRooms(ctx) {
  ...
  for (const room of rooms) {
    ...
    const conversationId = current?.id ?? (usedConversationIds.has(legacyConversationId) ? randomUUID3() : legacyConversationId);
...
function apply12(ctx, config, chain) {
  chain.middleware("allow_reply", async (session, context) => {
    ...
    const content = h4.select(session.elements, "text").join("").trimStart();
...
import { isMessageContentText as isMessageContentText2 } from "koishi-plugin-chatluna/utils/string";
  ...
  chain.middleware("censor", async (session, context) => {
    ...
    message.content = await Promise.all(
      baseContent.map((content) => {
        if (!isMessageContentText2(content)) {
          return content;
        }
        return {
          type: "text",
...
import { logger as logger8 } from "koishi-plugin-chatluna";
function apply14(ctx, config, chain) {
  ...
  async function oldChatLimitCheck(session, context) {
    ...
    if ((config.defaultModel === "无" || config.defaultModel.trim().length < 1) && ctx.chatluna.platform.listAllModels(ModelType.all).value.length < 1) {
      return session.text("chatluna.not_available_model");
    }
    let platformClient;
    try {
      ;
      [platformClient] = parseRawModelName2(model);
    } catch (e) {
      logger8.error(e);
      return session.text("chatluna.not_available_model");
    }
    ...
    if (!client.value) {
      logger8.error(`Can't find model adapter for ${model}`);
      return session.text("chatluna.not_available_model");
    }
...
function apply15(ctx, config, chain) {
  ...
  async function oldChatLimitSave(session, context) {
    ...
    let key = conversationId + "-" + session.userId;
    key = createHash3("md5").update(key).digest("hex");
    chatLimit.count++;
...
function apply18(ctx, config, chain) {
  ...
  chain.middleware("read_chat_message", async (session, context) => {
    ...
    if (typeof message === "string") {
      message = [h5.text(message)];
    }
    if (context.command == null) {
      const text = h5.select(message, "text").join("");
  ...
  chain.middleware("transform_chat_message", async (session, context) => {
    ...
    if (transformedMessage.content.length < 1 && getMessageContent6(transformedMessage.content).trim().length < 1) {
      return 1 /* STOP */;
    }
...
async function getFileConfig(ctx, model) {
  ...
  const [platform] = parseRawModelName3(model);
...
async function rollbackConversation(ctx, config, session, context, conversation, rollbackRound, resolved) {
  ...
  if (resolved.constraint.manageMode === "admin" && !await checkAdmin(session)) {
    return {
      status: 1 /* STOP */,
      msg: session.text(".conversation_not_exist")
    };
...
import { checkAdmin as checkAdmin2 } from "koishi-plugin-chatluna/utils/koishi";
...
function apply21(ctx, config, chain) {
  chain.middleware("stop_chat", async (session, context) => {
    ...
    if (resolved.constraint.manageMode === "admin" && !await checkAdmin2(session)) {
  ...
  chain.middleware("request_conversation", async (session, context) => {
    ...
    if (presetTemplate == null) {
      throw new ChatLunaError5(
        ChatLunaErrorCode5.PRESET_NOT_FOUND,
    ...
    try {
      ...
      [responseMessage] = await Promise.all([
        ctx.chatluna.conversationRuntime.chat(
          ...
          inputMessage,
          {
            ...
            stream,
            variables: {
              prompt: getMessageContent7(originContent),
              ...getSystemPromptVariables(
                session,
                config,
                conversation
      ...
      if (e?.message?.includes("output values have 1 keys")) {
        throw new ChatLunaError5(
          ChatLunaErrorCode5.MODEL_RESPONSE_IS_EMPTY
        );
      } else {
        throw e;
      }
...
function apply26(ctx, config, chain) {
  ...
  chain.middleware("list_all_embeddings", async (session, context) => {
    ...
    const models = service.listAllModels(ModelType2.embeddings).value.map((model) => model.toModelName());
...
function apply27(ctx, config, chain) {
  ...
  chain.middleware("list_all_model", async (session, context) => {
    ...
    const models = services.listAllModels(ModelType3.llm).value.map((model) => model.toModelName());
    await pagination.push(models);
    context.message = await pagination.getFormattedPage(page, limit);
    return 1 /* STOP */;
  }).after("lifecycle-handle_command").before("lifecycle-request_conversation");
...
function apply30(ctx, config, chain) {
  chain.middleware("resolve_model", async (session, context) => {
    ...
    try {
      ...
      const [platformName, rawModelName] = parseRawModelName4(modelName);
...
function apply31(ctx, config, chain) {
  ...
  chain.middleware("search_model", async (session, context) => {
    ...
    const models = services.listAllModels(ModelType5.llm).value.map((model) => model.toModelName());
    await pagination.push(models);
...
function apply32(ctx, config, chain) {
  ...
  chain.middleware("set_default_embeddings", async (session, context) => {
    ...
    const embeddings = service.listAllModels(ModelType6.embeddings);
...
function apply37(ctx, _config, chain) {
  chain.middleware("delete_preset", async (session, context) => {
    ...
    try {
      await fs4.rm(presetTemplate.path);
    } catch (e) {
      ctx.logger.error(e);
    }
    context.message = session.text(".success", [presetName]);
...
function apply39(ctx, config, chain) {
  chain.middleware("set_preset", async (session, context) => {
    ...
    await fs5.writeFile(preset.path, dump3(presetObject));
...
import { checkAdmin as checkAdmin3 } from "koishi-plugin-chatluna/utils/koishi";
...
function apply40(ctx, config, chain) {
  ...
  middleware2("conversation_new", async (session, context) => {
    ...
    if (resolved == null) {
      context.message = session.text(
        ...
        [ChatLunaErrorCode6.CONVERSATION_NOT_FOUND]
      );
      return 1 /* STOP */;
    }
    if (resolved.constraint.manageMode === "admin" && resolved.constraint.routeMode !== "personal" && !await checkAdmin3(session)) {
      context.message = session.text(
        "chatluna.conversation.messages.admin_required",
        [ChatLunaErrorCode6.CONVERSATION_ADMIN_REQUIRED]
      );
      return 1 /* STOP */;
    }
    ...
    for (const field of ["model", "preset", "chatMode"]) {
      ...
      if (value != null && fixed != null && value !== fixed) {
        context.message = session.text(
          ...
          [fixed, ChatLunaErrorCode6.CONVERSATION_FIXED]
        );
        return 1 /* STOP */;
      }
    }
  ...
  middleware2("conversation_export", async (session, context) => {
    ...
    try {
      ...
      try {
        const buffer = await fs6.readFile(result.path);
        await session.send(
          h7.file(buffer, "application/markdown", {
...
function actionLocked(session, action) {
  return session.text("chatluna.conversation.messages.action_locked", [
    ...
    ChatLunaErrorCode6.CONVERSATION_LOCKED
  ]);
}
...
function actionDisabled(session, action) {
  return session.text("chatluna.conversation.messages.action_disabled", [
    session.text(`chatluna.conversation.action.${action}`),
    ChatLunaErrorCode6.CONVERSATION_DISABLED
  ]);
}
...
function formatConversationError(session, error, action) {
  if (error instanceof ConversationResolutionError) {
    ...
    return session.text(
      ...
      [
        ambiguous ? ChatLunaErrorCode6.CONVERSATION_TARGET_AMBIGUOUS : ChatLunaErrorCode6.CONVERSATION_TARGET_OUTSIDE_ROUTE
      ]
    );
  }
  ...
  if (error instanceof ConstraintFixedError) {
    return session.text(
      ...
      [error.value, ChatLunaErrorCode6.CONVERSATION_FIXED]
    );
  }
  if (error instanceof InvalidChatModeError) {
    return session.text(
      "chatluna.conversation.messages.invalid_chat_mode",
      [error.mode, ChatLunaErrorCode6.CONVERSATION_INVALID_CHAT_MODE]
    );
  }
  if (error instanceof ChatLunaError6) {
...
function apply42(ctx, _config, chain) {
  ...
  chain.middleware("wipe", async (session, context) => {
    ...
    try {
      await fs7.rm("data/chathub/vector_store", { recursive: true });
    } catch (e) {
      logger13.warn(`wipe: ${e}`);
    }
    try {
      await fs7.rm("data/chatluna/temp", { recursive: true });
    } catch (e) {
      logger13.warn(`wipe: ${e}`);
    }
    ...
    await fs7.mkdir(sentinelDir, { recursive: true });
    await fs7.writeFile(sentinelPath, "{}", "utf8");
    context.message = session.text(".success");
    const appContext = ctx.scope.parent;
    appContext.scope.update(appContext.config, true);
    return 1 /* STOP */;
...
var Config2 = Schema.intersect([
  ...
  Schema.intersect([
    ...
    Schema.union([
      Schema.object({
        ...
        replyQuoteThreshold: Schema.number().min(0).max(600).step(1).default(0)
      }),
      Schema.object({})
    ])
  ]),
  ...
  Schema.object({
    ...
    infiniteContextThreshold: Schema.percent().min(0.5).max(0.95).step(0.01).default(0.85),
    autoArchive: Schema.boolean().default(false),
    autoArchiveTimeout: Schema.number().default(Time2.day * 10 / Time2.second).min(Time2.hour / Time2.second),
    autoPurgeArchive: Schema.boolean().default(false),
    autoPurgeArchiveTimeout: Schema.number().default(Time2.day * 30 / Time2.second).min(Time2.hour / Time2.second)
  }),
  Schema.object({
...
import {
  getMessageContent as getMessageContent8,
  transformMessageContentToElements as transformMessageContentToElements2
} from "koishi-plugin-chatluna/utils/string";
var TextRenderer = class extends Renderer {
  ...
  async render(message, options) {
    if (options.session != null && options.session.platform === "qq" && options.session.isDirect) {
      return {
        element: [
          h8.text(getMessageContent8(message.content)),
          ...h8.parse("<markdown-qq></markdown-qq>")
        ]
      };
    }
    let transformed = transformMessageContentToElements2(message.content);
    transformed = transformAndEscape(
      transformed,
      options.session?.platform ?? "sandbox"
    );
    if (options.split) {
      transformed = transformed.map((element) => h8("message", element));
    }
...
var VoiceRenderer = class extends Renderer {
  ...
  async render(message, options) {
    const baseElements = transformMessageContentToElements3(message.content);
...
var RawRenderer = class extends Renderer {
  ...
  async render(message, options) {
    ...
    return {
      element: transformMessageContentToElements4(message.content)
    };
  }
...
var KoishiElementRenderer = class extends Renderer {
  ...
  async render(message, options) {
    let transformed = transformMessageContentToElements5(message.content);
...
var MixedVoiceRenderer = class extends Renderer {
  ...
  async render(message, options) {
    const elements = [];
    const baseElements = transformMessageContentToElements6(message.content);
...
// src/renders/pure-text.ts
import he2 from "he";
import { transformMessageContentToElements as transformMessageContentToElements7 } from "koishi-plugin-chatluna/utils/string";
var PureTextRenderer = class extends Renderer {
  static {
    __name(this, "PureTextRenderer");
  }
  async render(message, options) {
    let transformed = transformMessageContentToElements7(message.content);
    if (options.split) {
