...
function repairValue(schema, value) {
  ...
  if (schema instanceof z.ZodString && (typeof value === "number" || typeof value === "boolean")) {
    return String(value);
  }
...
async function compressChunk(model, transcript, conversationId, signal, instruction) {
  ...
  const text = (result["text"] ?? "").toString().trim() || (rawMessage ? getMessageContent2(rawMessage.content).trim() : "");
  if (!text) {
    return null;
  }
  return {
...
async function* runAgent(options) {
  ...
  while (iterations < maxIterations) {
    ...
    if (last != null && (tool?.returnDirect || isDirectToolOutput2(last.observation))) {
      yield {
        type: "round-decision",
        canContinue: false
      };
...
${scratchPart}` : scratchPart;
  ...
  try {
    ...
    logger2.error("[ScratchpadCompress] Failed:", e);
  }
}
...
var LegacyAgentExecutor = class _LegacyAgentExecutor extends BaseChain {
  static {
    ...
    for await (const event of runner) {
      ...
      if (event.type === "done") {
        ...
        return {
          ...
          message: event.message ?? new AIMessage2(event.output)
        };
      }
    }
...
function _convertAgentStepToMessages(action, observation) {
  if (isToolsAgentAction(action) && action.toolCallId !== void 0) {
    ...
    return action.messageLog?.concat(
      new FunctionMessage(
        getMessageContent3(observation),
        action.tool
      )
    );
  } else {
    return [new AIMessage3(action.log)];
  }
}
...
function createOpenAIAgent({
  ...
  const agent = RunnableSequence.from([
    ...
    RunnableLambda.from((input) => {
      ...
      const hasTools = input.additional_kwargs?.tool_calls?.length > 0 || (input instanceof AIMessageChunk3 || input instanceof AIMessage3) && input.tool_calls?.length > 0;
      ...
      return outputParser.parseResult([
        {
          message: input,
          text: getMessageContent3(input.content)
        }
      ]);
    })
  ]);
...
import { getMessageContent as getMessageContent5 } from "koishi-plugin-chatluna/utils/string";
...
function createReactAgent({
  ...
  const instructionsFormat = PromptTemplate2.fromTemplate(
    ...
    [
      ...
      RunnableLambda2.from(async (input) => {
        const result = await outputParser.parse(
          getMessageContent4(input.content) ?? ""
        );
        if (!Array.isArray(result) && "returnValues" in result) {
          return {
            ...result,
...
import { Runnable as Runnable2 } from "@langchain/core/runnables";
var AgentRunner = class _AgentRunner extends Runnable2 {
  static {
  ...
  async *_run(input, config, runManager) {
    ...
    for await (const event of runAgent({
      ...
      yield {
        ...
        message: event.message ?? new AIMessage4(event.output)
      };
      return;
    }
...
function createAgent(options) {
  ...
  const agent = {
    ...
    async generate(input) {
      const message = typeof input.prompt === "string" ? new HumanMessage3(input.prompt) : input.prompt;
      const text = getMessageContent5(message.content);
...
function createAgentTool(agent, options = {}) {
  return new AgentTool({
    ...
    schema: options.schema ?? z2.object({
      prompt: z2.string().describe("The complete task to delegate to this agent")
    })
  });
}
__name(createAgentTool, "createAgentTool");
function createPromptPreset(name, system, preset) {
  return preset ?? computed2(
    () => ({
      triggerKeyword: [name],
      rawText: system ?? "",
...
var AgentTool = class extends StructuredTool2 {
  static {
...
import { computed as computed3 } from "@vue/reactivity";
...
function resolveAgentTools(input, getTool) {
  ...
  if (typeof input[0] === "string") {
    return computed3(() => input.map((name) => getTool(name)));
  }
  return computed3(() => input);
}
...
import {
  AIMessage as AIMessage5,
  HumanMessage as HumanMessage4,
  ToolMessage as ToolMessage2
} from "@langchain/core/messages";
import { logger as logger3 } from "koishi-plugin-chatluna";
import { getMessageContent as getMessageContent7 } from "koishi-plugin-chatluna/utils/string";

// src/llm-core/agent/sub-agent/utils.ts
import { getMessageContent as getMessageContent6 } from "koishi-plugin-chatluna/utils/string";
...
function formatTaskHistory(messages) {
  const lines = messages.map((msg) => {
    const text = getMessageContent6(msg.content).replace(/\s+/g, " ").trim();
...
async function runAgentTask(options) {
  ...
  const promptMessage = new HumanMessage4(options.prompt);
  const snapshot = isBg ? {
  ...
  run.trace.push({
    ...
    at: Date.now(),
    title: "用户请求",
    text: getMessageContent7(promptMessage.content)
  });
  ...
  const exec = /* @__PURE__ */ __name(async () => {
    ...
    try {
      ...
      if (getMessageContent7(result.message.content).trim().length > 0) {
        saveUser();
  ...
  if (isBg) {
    exec().catch((err) => {
      ...
      logger3.error("[SubagentBgTaskError]", err);
    });
...
async function notifyFinished(options, run, snapshot) {
  ...
  const message = new HumanMessage4(
...
async function onTaskEvent(task, run, saveUser, event) {
  ...
  if (event.type === "human-update") {
    ...
    for (const item of event.messages) {
      run.trace.push({
        ...
        at: Date.now(),
        title: "追加消息",
        text: getMessageContent7(item.content)
      });
    }
  }
  ...
  if (event.type === "done") {
    run.trace.push({
      ...
      at: Date.now(),
      title: "最终输出",
      text: (event.replyEmitted ? "最终回复已由工具发送。" : "") || getMessageContent7(event.message?.content ?? "") || event.output || event.log
    });
  }
}
...
import { StructuredTool as StructuredTool3 } from "@langchain/core/tools";
...
function renderAvailableAgents(agents, dir, location = "local") {
  ...
  return new SystemMessage2(lines.join("\n"));
}
__name(renderAvailableAgents, "renderAvailableAgents");
var AgentTaskTool = class extends StructuredTool3 {
  ...
  description;
  schema = z3.object({
    action: z3.enum(["run", "status", "list", "list_all", "message", "stop"]).optional().describe(
    ...
    agent: z3.string().optional().describe(
      "Exact agent name. Required for new tasks; optional when resuming by id."
    ),
    id: z3.string().optional().describe("Existing task id for status, message, or resume."),
    prompt: z3.string().optional().describe(
      "Task or follow-up instruction. Required for run. Use goal for the structured Markdown objective."
    ),
    goal: z3.string().optional().describe(
      "Markdown goal block for the sub-agent. Include objective, scope, success criteria, expected output, and constraints."
    ),
    reason: z3.string().optional().describe("Optional reason for delegating the task"),
    background: z3.boolean().optional().describe("Run in background for long work."),
    message: z3.string().optional().describe("Guidance for a running background task.")
  }).superRefine((val, ctx) => {
...
function createTaskTool(options) {
  ...
  const stopTaskRun = /* @__PURE__ */ __name((runId) => {
    ...
    item.resume?.();
    item.abort.abort(
      new ChatLunaError(ChatLunaErrorCode.ABORTED, void 0, true)
    );
    return true;
  }, "stopTaskRun");
  const runtime = {
    ...
    async chatTask(id, prompt, ctx) {
      ...
      const item = runId ? active.get(runId) : void 0;
      if (runId && item?.queue) {
        item.queue.push(new HumanMessage5(prompt));
        task.updatedAt = Date.now();
    ...
    async runTask(input, runConfig) {
      ...
      if (action === "message") {
        ...
        item.queue.push(new HumanMessage5(input.message.trim()));
        task.updatedAt = Date.now();
        scheduleTaskCleanup(task.id);
        await refreshQuietly();
        return [
