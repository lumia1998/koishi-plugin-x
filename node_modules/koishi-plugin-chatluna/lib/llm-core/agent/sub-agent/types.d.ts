...
export interface AgentTaskRunTraceEntry {
    id: string;
    type: 'prompt' | 'message' | 'thought' | 'tool-call' | 'tool-result' | 'output' | 'error';
    at: number;
    text: string;
    tool?: string;
    title?: string;
...
export interface CreateTaskToolOptions {
    ...
    get: (name: string, ctx: AgentTaskResolveContext) => Awaitable<AgentTaskTarget | undefined>;
...
export interface AgentTaskToolRuntime {
    ...
    chatTask(id: string, prompt: string, ctx: AgentTaskResolveContext): Promise<{
        state: 'queued' | AgentTaskRun['state'];
        output?: string;
    }>;
    runTask(input: AgentTaskInput, runConfig?: ChatLunaToolRunnable): Promise<string>;
}
...
declare module 'koishi' {
    interface Events {
        'chatluna/agent-task-finished': (payload: AgentTaskFinishedPayload) => Promise<void>;
    }
}
