...
__export(prompt_exports, {
  ...
  countMessageTokens: () => import_count_tokens.countMessageTokens,
  countMessagesTokens: () => import_count_tokens.countMessagesTokens,
  createAfterUserMessageMiddleware: () => createAfterUserMessageMiddleware,
  createAuthorsNoteMiddleware: () => createAuthorsNoteMiddleware,
  createChatHistoryMiddleware: () => createChatHistoryMiddleware,
  createInjectionsMiddleware: () => createInjectionsMiddleware,
  createLongHistoryMiddleware: () => createLongHistoryMiddleware,
  createLoreBooksMiddleware: () => createLoreBooksMiddleware,
  createReadFilesContextMiddleware: () => createReadFilesContextMiddleware,
  createSystemPromptsMiddleware: () => createSystemPromptsMiddleware,
  findMessageIndex: () => findMessageIndex,
  isRoleBook: () => isRoleBook,
  isRoleBookConfig: () => isRoleBookConfig,
  loadPreset: () => loadPreset,
  registerAfterUserMessageMiddleware: () => registerAfterUserMessageMiddleware,
  registerAuthorsNoteMiddleware: () => registerAuthorsNoteMiddleware,
  registerChatHistoryMiddleware: () => registerChatHistoryMiddleware,
  registerInjectionsMiddleware: () => registerInjectionsMiddleware,
  registerLongHistoryMiddleware: () => registerLongHistoryMiddleware,
  registerLoreBooksMiddleware: () => registerLoreBooksMiddleware,
  registerReadFilesContextMiddleware: () => registerReadFilesContextMiddleware,
  registerSystemPromptsMiddleware: () => registerSystemPromptsMiddleware,
...
function createSystemPromptsMiddleware() {
  ...
  return async (runtime, next) => {
    ...
    if (runtime.instructions) {
      const msg = new import_messages3.SystemMessage(runtime.instructions);
      const tokens = await (0, import_count_tokens.countMessageTokens)(msg, runtime.tokenCounter);
    ...
    for (const message of rendered.messages ?? []) {
      const tokens = await (0, import_count_tokens.countMessageTokens)(
        message,
    ...
    if (_cachedPreset !== preset) {
      _cachedSummaryPrompt = import_prompts.HumanMessagePromptTemplate.fromTemplate(
...
function createChatHistoryMiddleware() {
  return async (runtime, next) => {
    ...
    if (runtime.input) {
      const inputTokens = await (0, import_count_tokens.countMessageTokens)(
        runtime.input,
        runtime.tokenCounter
      );
    ...
    if (runtime.agentScratchpad) {
      if (Array.isArray(runtime.agentScratchpad)) {
        for (const msg of runtime.agentScratchpad) {
          runtime.usedTokens += await (0, import_count_tokens.countMessageTokens)(
            msg,
            runtime.tokenCounter
          );
        }
    ...
    for (let i = rounds.length - 1; i >= 0; i--) {
      ...
      for (const msg of round) {
        roundTokens += await (0, import_count_tokens.countMessageTokens)(
          msg,
          runtime.tokenCounter
        );
      }
    ...
    if (rounds.length > 0 && selectedRounds.length === 0) {
      ...
      for (const msg of lastRound) {
        usedTokens += await (0, import_count_tokens.countMessageTokens)(
          msg,
          runtime.tokenCounter
        );
      }
    ...
    if (truncated && hasValidLimit) {
      import_koishi_plugin_chatluna3.logger?.warn(
...
function buildConversationRounds(messages) {
  ...
  for (const message of messages) {
    const isStart = (0, import_langchain.isChatLunaUserMessage)(message) || message.getType() === "human";
    if (isStart) {
      if (current.length > 0) {
        rounds.push(current);
      }
...
async function formatLoreBooks(loreBooks, usedTokens, result, variables, runtime) {
  ...
  const loreBooksPrompt = import_prompts2.HumanMessagePromptTemplate.fromTemplate(
    preset.config.loreBooksPrompt ?? "{input}"
  );
  ...
  for (const loreBook of loreBooks) {
    ...
    if (loreBookTokens > tokenLimit) {
      import_koishi_plugin_chatluna4.logger?.warn(
    ...
    if (position === "default") {
      if (hasLongMemory) {
        const index = result.findIndex(
          (msg) => msg instanceof import_messages4.AIMessage && msg.content === "Ok. I will remember."
        );
  ...
  return async (context, next) => {
    ...
    if (rawPosition === "in_chat") {
      ...
      runtime.result.splice(
        safeInsertPosition,
        0,
        new import_messages5.HumanMessage(formatAuthorsNote)
      );
    } else {
      runtime.result.splice(
        insertPosition,
...
function createReadFilesContextMiddleware() {
  return async (context, next) => {
    ...
    const messages = Array.isArray(value) ? value : value instanceof import_messages6.BaseMessage ? [value] : [];
    if (messages.length === 0) {
      return next();
    }
