    ...
    try {
        ...
        if (response && response.tweet) {
            ...
            const mediaElements = mediaUrls
                .filter((url) => !url.includes('.mp4') && !url.includes('video.twimg.com'))
...
async function scrapeTweetWithCookie(ctx, url, cookies, maxRetries = 3) {
    let attempts = 0;
    let page;
            ...
            if (!isProtected) {
                ...
                try {
                    ...
                    if (apiResponse && apiResponse.media_extended) {
                        mediaUrls = apiResponse.media_extended.map((m) => m.url);
                    }
                }
                catch (apiErr) {
