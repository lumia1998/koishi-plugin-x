...
exports.Config = exports.inject = exports.logger = exports.name = void 0;
exports.apply = apply;
const koishi_1 = require("koishi");
exports.name = 'x';
exports.logger = new koishi_1.Logger('x');
exports.inject = { required: ['puppeteer'], optional: ['chatluna'] };
...
exports.Config = koishi_1.Schema.intersect([
    koishi_1.Schema.object({
        cookies: koishi_1.Schema.string().required().description('Twitter/X 登录 Cookie (auth_token)'),
        fetchRetries: koishi_1.Schema.number().min(1).default(3).description('抓取推文失败时的重试次数')
    }).description('基础设置'),
    koishi_1.Schema.object({
        whe_translate: koishi_1.Schema.boolean().default(false).description('是否启用推文翻译（通过 ChatLuna 调用大模型）')
    }).description('翻译设置'),
    koishi_1.Schema.union([
        koishi_1.Schema.object({
            whe_translate: koishi_1.Schema.const(true).required(),
            model: koishi_1.Schema.dynamic('model').description('使用的大语言模型名称 (需要通过 ChatLuna 先配置好)'),
            prompt: koishi_1.Schema.string().role('textarea').default(DEFAULT_PROMPT).description('翻译使用的提示词，使用{text}表示需要翻译的文本'),
            translateRetries: koishi_1.Schema.number().min(1).default(3).description('翻译接口失败时的重试次数')
        }),
        koishi_1.Schema.object({}),
    ]),
    koishi_1.Schema.object({
        outputLogs: koishi_1.Schema.boolean().default(true).description('日志调试模式'),
        detectXLinks: koishi_1.Schema.boolean().default(true).description('是否启用 X/Twitter 链接检测'),
        useForward: koishi_1.Schema.boolean().default(false).description('是否使用合并转发形式发送（仅 QQ/OneBot 平台有效）')
    }).description('调试设置'),
]);
...
function safeImageSrc(value) {
    ...
    if (/^data:image\/(?:png|jpe?g|webp|gif);base64,[a-z0-9+/=]+$/i.test(url))
        return url;
    if (/^https?:\/\//i.test(url))
        return url;
    return '';
...
function generateTweetHTML(authorName, screenName, avatarUrl, tweetText, mediaPreviewUrls) {
...
async function apply(ctx, config) {
    ...
    const loadModel = async () => {
        try {
            if (config.whe_translate && config.model) {
                if (!ctx.chatluna) {
                    exports.logger.warn('已启用翻译，但 ChatLuna 服务不可用。请安装并启用 koishi-plugin-chatluna。');
                    return;
                }
            ...
            exports.logger.error('加载 ChatLuna 模型时出错：', e);
        }
    };
    ctx.on('ready', async () => {
        await loadModel();
    });
    async function sendProcessedTweet(sessionParam, message) {
        ...
        if (config.useForward) {
            await sessionParam.send((0, koishi_1.h)('figure', {}, finalParts));
            return;
        }
    ...
    async function downloadImageElement(imageUrl) {
    ...
    async function downloadVideoElement(videoUrl, timeout) {
        ...
        return koishi_1.h.video(videoResponse, 'video/mp4');
    }
    async function retryMediaElement(label, url, task, maxRetries = 3) {
        let attempts = 0;
        while (attempts < maxRetries) {
            try {
                ...
                attempts++;
                exports.logger.error(`${label}失败，正在尝试第 ${attempts} 次重试: ${url}`, error);
                if (attempts >= maxRetries) {
                    exports.logger.error(`${label}失败，已达最大重试次数: ${url}`, error);
                    return null;
                }
            }
        }
    ...
    async function renderApiTweetCard(tpTweet, tweetWord, altOriginalText) {
        if (!tpTweet.fromAPI || !tpTweet.authorName || !tpTweet.authorAvatar)
            return null;
        if (config.outputLogs)
            exports.logger.info('开始渲染 HTML 卡片模式');
        try {
            const imageUrls = tpTweet.imageUrls || [];
            const videoUrls = tpTweet.videoUrls || [];
            if (config.outputLogs)
                exports.logger.info(`媒体数量: 图片${imageUrls.length}, 视频${videoUrls.length}`);
            ...
            for (const imgUrl of imageUrls.slice(0, 4)) {
                try {
                    ...
                    if (config.outputLogs)
                        exports.logger.warn('下载图片预览失败:', err);
                }
            }
            for (const videoUrl of videoUrls) {
                ...
                try {
                    ...
                    if (config.outputLogs)
                        exports.logger.warn('下载视频缩略图失败:', err);
                }
            }
            if (config.outputLogs)
                exports.logger.info('开始下载头像');
            ...
            if (config.outputLogs)
                exports.logger.info('生成 HTML 模板');
            const html = generateTweetHTML(tpTweet.authorName, tpTweet.screenName, avatarDataUrl, tweetWord, mediaPreviewUrls);
            if (config.outputLogs)
                exports.logger.info('开始渲染 HTML 为图片');
            ...
            try {
                ...
                if (config.outputLogs)
                    exports.logger.info('HTML 卡片渲染成功');
            }
            finally {
                await page.close();
            }
            const parts = [koishi_1.h.image(cardBuffer, 'image/png')];
            if (config.outputLogs)
                exports.logger.info(`准备下载 ${videoUrls.length} 个视频`);
            for (const videoUrl of videoUrls) {
                const videoElement = await retryMediaElement('下载视频', videoUrl, () => downloadVideoElement(videoUrl, 60000));
                if (videoElement) {
                    parts.push(videoElement);
                    if (config.outputLogs)
                        exports.logger.info(`成功下载视频: ${videoUrl}`);
                }
            }
            return { parts };
        }
        catch (err) {
            exports.logger.error('渲染 HTML 卡片失败，降级到文字模式:', err);
            return { parts: [tweetWord + altOriginalText] };
        }
    }
    async function buildTweetMessage(tpTweet, tweetWord, altOriginalText) {
        ...
        if (tpTweet.authorAvatar) {
            try {
                ...
                parts.push(koishi_1.h.image(avatarResponse, 'image/jpeg'));
            }
            catch (err) {
                if (config.outputLogs)
                    exports.logger.warn('获取博主头像失败:', err);
            }
        }
        if (tpTweet.screenshotBuffer) {
            parts.push(koishi_1.h.image(tpTweet.screenshotBuffer, "image/webp"));
        }
        if (tweetWord) {
        ...
        const imageElements = await Promise.all(imageUrls.map((imageUrl) => retryMediaElement('请求图片', imageUrl, () => downloadImageElement(imageUrl))));
        ...
        for (const videoUrl of videoUrls) {
            ...
            if (videoElement) {
                parts.push(videoElement);
                if (config.outputLogs)
                    exports.logger.info(`成功请求视频文件: ${videoUrl}`);
            }
        }
        return { parts };
    }
        ...
        try {
            ...
            if (config.whe_translate === true && config.model) {
                try {
                    ...
                    if (config.outputLogs) {
                        exports.logger.info("手动查询翻译结果：", translation_result);
                    }
                    tweetWord = translation_result;
                }
                catch (err) {
                    exports.logger.error("手动翻译失败，返回原文：", err);
                    tweetWord = tweetText;
                }
            }
            else {
            ...
            exports.logger.info("获取推文过程失败", error);
        }
    }
    ...
    const expandShortLink = async (url) => {
        try {
            const res = await ctx.http.get(url, { redirect: 'manual' });
    ...
    ctx.middleware(async (session2, next) => {
        try {
            if (!config || config.detectXLinks === false)
                return next();
            ...
            if (found.length) {
                exports.logger.info('检测到 X/Twitter 链接:', found);
                for (const link of found) {
                    try {
                        await processTwitterUrl(session2, link);
                    }
                    catch (e) {
                        exports.logger.error('处理检测到的 X/Twitter 链接时出错', e);
                    }
                }
            }
        }
        catch (err) {
            exports.logger.error('X/Twitter 链接检测失败', err);
        }
        return next();
    });
}
async function getTimePushedTweet(ctx, pptr, url, config, maxRetries) {
    ...
    while (apiAttempts < retryLimit) {
        try {
            if (config.outputLogs)
                exports.logger.info(`尝试 fxtwitter API (${apiAttempts + 1}/${retryLimit}): ${fxUrl}`);
            ...
            if (apiResponse && apiResponse.tweet) {
                ...
                let altTexts = [];
                let mediaUrls = [];
                let videoUrls = []; // 单独保存视频URL
                let imageUrls = []; // 单独保存图片URL
                let videoThumbnails = {}; // 视频URL -> 缩略图URL
                // 提取媒体和 alt 文本
                ...
                // API 成功，不需要截图
                if (config.outputLogs)
                    exports.logger.info('fxtwitter API 获取成功');
                return {
            ...
            apiAttempts++;
            exports.logger.warn(`fxtwitter API 请求失败 (${apiAttempts}/${retryLimit}):`, err.message || err);
            if (apiAttempts >= retryLimit) {
                exports.logger.warn('fxtwitter API 重试次数已用尽，降级使用 puppeteer 截图');
                break;
            }
            // 指数退避
    ...
    exports.logger.info('开始使用 puppeteer 截图模式');
    let page;
    let attempts = 0;
    while (attempts < retryLimit) {
        try {
            ...
            try {
                ...
                if (config.outputLogs) {
                    try {
                        ...
                        exports.logger.error('等待 article 超时，已保存调试截图（页面可能需要代理或 Cookie 失效）');
                        exports.logger.error('页面 HTML 长度:', html.length);
                        exports.logger.error('页面 title:', await page.title());
                        // 检查是否有登录提示或错误信息
                        const bodyText = await page.evaluate(() => document.body?.innerText || '');
                        exports.logger.error('页面文本内容（前500字符）:', bodyText.substring(0, 500));
                    }
                    catch { /* ignore */ }
                }
            ...
            await page.evaluate(async () => {
                ...
                await Promise.all(videos.map(video => {
                    if (video.readyState >= 2)
                        return Promise.resolve(); // HAVE_CURRENT_DATA
                    return new Promise(resolve => {
                ...
                // 媒体加载超时也继续，避免卡死
                if (config.outputLogs)
                    exports.logger.warn('媒体加载超时，继续截图');
            });
            // 额外等待 1 秒让动画稳定
            ...
            if (isProtected) {
                ...
                while (vxApiAttempts < retryLimit) {
                    try {
                        ...
                        vxApiAttempts++;
                        exports.logger.error(`请求 vxtwitter API 失败，正在尝试第 ${vxApiAttempts} 次重试...`, err);
            ...
            attempts++;
            exports.logger.error(`获取推文内容失败，正在尝试第 ${attempts} 次重试...`, error);
            if (attempts >= retryLimit) {
                exports.logger.error(`获取推文内容失败，已达最大重试次数。推文链接：${url}`, error);
                return {
                    word_content: '',
                    altTexts: [],
                    mediaUrls: [],
...
async function translate(text, ctx, config, chatLunaModel) {
    ...
    while (attempts < retryLimit) {
        try {
            ...
            if (config.outputLogs) {
                exports.logger.info('翻译api返回结果：', response);
            }
            ...
            attempts++;
            exports.logger.error(`翻译失败，正在尝试第 ${attempts} 次重试...`, err);
            if (attempts >= retryLimit) {
                exports.logger.error('翻译失败，请检查 ChatLuna 配置是否正确：', err);
