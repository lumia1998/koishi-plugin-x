...
function restoreHashtags(text, hashtags) {
    let restored = text;
...
async function getTweetScreenshot(puppeteer, url, cookie, outputLogs) {
    const page = await puppeteer.page();
    try {
        if (outputLogs)
            exports.logger.info(`开始截取真实 X 推文: ${url}`);
        if (cookie) {
        ...
        if (!tweetElement)
            throw new Error('无法在页面上定位到推文元素');
        if (outputLogs)
            exports.logger.info('已定位真实推文元素，开始截图');
    ...
    async function buildTweetMessage(tpTweet, tweetWord, altOriginalText) {
        ...
        if (tpTweet.url) {
            try {
                ...
                parts.push(koishi_1.h.image(screenshotBuffer, 'image/png'));
                hasRenderedPreview = true;
            }
            catch (err) {
                exports.logger.warn('真实推文截图失败，降级使用已有截图或文字媒体模式:', err);
            }
        }
